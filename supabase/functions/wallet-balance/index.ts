...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (error) return json(500, { error: 'Failed to load wallet state', details: error.message });
    return json(200, { state: data });
  } catch (error: any) {
    if (String(error?.message || '').includes('Unauthorized')) return json(401, { error: 'Unauthorized' });
    return json(500, { error: 'Unexpected wallet-balance error', details: error?.message || error });
  }
});
